  ...
  return (
    <Section id="capabilities" className="bg-[#0B1728] text-white">
      <div className="max-w-4xl mx-auto text-center mb-14">
        <span className="text-[var(--color-brand-accent)] font-semibold tracking-[0.2em] uppercase text-xs">Material & Process Deep-Dive</span>
        <h2 className="text-3xl md:text-5xl font-bold mt-2 tracking-tight">Capabilities & Specifications</h2>
        <p className="text-slate-400 text-sm md:text-base mt-3 font-sans">Click any discipline to view material compatibility, tolerance thresholds, and real case examples.</p>
      </div>
      ...
      <div className="max-w-5xl mx-auto space-y-3">
        {capabilities.map((cap) => {
          ...
          return (
            <div key={cap.id} className={`rounded-xl border overflow-hidden transition-all duration-300 ${ isOpen ? "border-[var(--color-brand-accent)]/50 bg-white/[0.06]" : "border-white/10 bg-white/[0.03] hover:border-white/20" }`}>
              <button onClick={() => setOpenId(isOpen ? null : cap.id)} className="w-full px-6 py-5 text-left flex items-center justify-between space-x-4 focus:outline-none">
              ...
              {isOpen && (
                <div className="px-6 pb-6 pt-2 border-t border-white/10 grid grid-cols-1 md:grid-cols-4 gap-6 text-sm font-sans">
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">{cap.materials.map((mat, i) => (<li key={i} className="text-slate-300 text-xs flex items-center"><span className="w-1 h-1 rounded-full bg-[var(--color-brand-accent)] mr-2" />{mat}</li>))}</ul>
                  </div>
                  <div>
                    <span className="block text-[10px] uppercase font-bold text-slate-500 tracking-wider mb-2">Tolerance</span>
                    <span className="inline-block bg-[var(--color-brand-accent)]/20 text-[var(--color-brand-accent)] font-mono font-bold px-3 py-1.5 rounded text-xs border border-[var(--color-brand-accent)]/30">{cap.tolerance}</span>
                  </div>
                  <div>
