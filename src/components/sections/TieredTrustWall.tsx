...
const logoTiers = [
  {
    ...
    logos: [
      ...
      { name: "Bellatrix", src: "/client_logos/new/bellatrix.png", zoomClass: "p-2" },
...
export default function TieredTrustWall() {
  ...
  return (
    <Section id="clients" ref={sectionRef} className="bg-white text-[var(--color-deep-navy)] py-24 relative overflow-hidden border-t border-slate-200">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 relative z-10 max-w-6xl mx-auto px-6 gap-6">
        <div className="max-w-2xl text-left">
          ...
          <h2 className="text-3xl md:text-5xl font-bold mt-4 tracking-tight text-[var(--color-deep-navy)] leading-tight">
        ...
        {logoTiers.map((tier, tIdx) => (
          <div key={tIdx}>
            ...
            <div className="flex items-center space-x-4 mb-8">
              ...
              <h3 className="text-[10px] sm:text-xs md:text-sm font-bold uppercase tracking-[0.1em] sm:tracking-[0.2em] text-[#00b5e2] text-center whitespace-normal sm:whitespace-nowrap bg-slate-100/80 px-4 py-2 rounded-2xl sm:rounded-full border border-slate-200 max-w-[85vw] sm:max-w-none leading-relaxed mx-auto">
                {tier.title}
              </h3>
            ...
            <div className="flex flex-wrap justify-center items-center gap-6 md:gap-8">
              {tier.logos.map((logo, lIdx) => {
                return (
                  ...
                  >
                    <div
                      className={`relative w-32 h-32 md:w-36 md:h-36 flex items-center justify-center rounded-full bg-gradient-to-tr from-white/90 via-white/70 to-white/90 backdrop-blur-xl border border-slate-200 shadow-[inset_0_2px_6px_rgba(255,255,255,0.9),_0_8px_32px_rgba(0,0,0,0.10)] transition-all duration-300 hover:scale-105 hover:-translate-y-1.5 hover:shadow-[inset_0_2px_6px_rgba(255,255,255,0.9),_0_0_35px_rgba(0,181,226,0.4)] hover:border-[#00b5e2]/60 cursor-pointer ${logo.zoomClass || 'p-4'}`}
                    >
                      <Image
                        src={logo.src}
                        alt={logo.name}
                    ...
                    <span className="text-xs md:text-sm text-slate-600 mt-3 font-sans text-center font-bold tracking-wide group-hover:text-[#00b5e2] transition-colors">
                      {logo.name}
                    </span>
                  </div>
                );
