      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProducts.map((product) => (
            <div key={product.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-md flex flex-col justify-between hover:border-[#00b5e2]/40 transition-all duration-300">
              <div className="relative w-full h-56 bg-slate-50 border-b border-slate-100 p-6 flex items-center justify-center">
                <Image
                  src={product.img}
                  alt={product.title}
                  fill
              ...
              <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                <div>
                  <h3 className="text-lg font-bold text-slate-800 mb-2 leading-snug">{product.title}</h3>
                  <p className="text-xs text-slate-500 font-sans leading-relaxed">{product.desc}</p>
                </div>
                ...
                <div className="pt-4 border-t border-slate-100 space-y-2.5">
                  <div className="flex justify-between items-center text-xs text-slate-700 font-sans">
                    ...
                    <span className="font-semibold text-slate-700">{product.material}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs text-slate-700 font-sans">
                    <span className="text-slate-400 font-bold">Tolerance:</span>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-100">
                    <div className="flex flex-wrap gap-1">
                      {product.tags.map((tag, idx) => (
                        <span key={idx} className="text-[10px] font-semibold bg-slate-50 text-slate-600 border border-slate-200 px-2 py-0.5 rounded">
                          {tag}
                        </span>
                      ))}
                    </div>
